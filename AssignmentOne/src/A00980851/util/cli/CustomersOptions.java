 ...
 * Class  Name     CustomersOptions
 * Date            2017-05-31
 * reads the customer arguments if present and acts on them
 */
    ...
    public void customersCliCheck(String[] args, LinkedList<Customer> customerLinkedList) throws ParseException {
        ...
        if (SetupCLI.hasCustomers()) {
            ...
            if (SetupCLI.isDescenindingAny()) {
                customerLinkedList.sort((s1, s2) -> s1.getJoinDate().compareTo(s2.getJoinDate()) * -1); // comapres by first name in descending order
            }
