 ...
 * MotorCycleReader Date 2017-05-28 Motorcycle.dat reading class
 */
public class MotorCycleReader extends StringPrinting {
	 ...
	 */
	public LinkedList<Motorcycle> parse() throws ApplicationException {
	...
	private LinkedList<String> readFile(Scanner scanner) {
		...
		LOG.debug(String.format("added  %s motorcycle objects to list from file", customers.size()));
		return customers;
	}
	 ...
	 * @throws ApplicationException
	 */
	private LinkedList<Motorcycle> praseStrings(LinkedList<String> customerStrings) throws ApplicationException {

		LinkedList<Motorcycle> motorCycle = new LinkedList<>();
		Motorcycle[] motorcycles = new Motorcycle[7];
	...
	private Motorcycle makeInventory(String[] motorCycle) throws ApplicationException {
		...
		if (motorCycle.length != 7) {
			throw new ApplicationException(String.format(
					"Arguments length not correct %s - MotorCycleReader Class System Exited", getString(motorCycle)));
		}
