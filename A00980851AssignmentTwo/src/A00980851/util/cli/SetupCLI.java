...
/**
 * Name Japneet Johal A00 980 851 Project Name AssignmentOne Class Name opt Date
 * 2017-05-30
 */
public class SetupCLI {
	...
	private static Options createOptions() {
		...
		options.addOption(
				Option.builder(Value.byCount.option).longOpt(Value.byCount.longoption).desc("byCount").build());
		options.addOption(Option.builder(Value.byJoinDate.option).longOpt(Value.byJoinDate.longoption)
				.desc("byJoinDate").build());
		options.addOption(
				Option.builder(Value.make.option).longOpt(Value.make.longoption).desc("make").hasArg().build());
	...
	public enum Value {
		service("s", "service"), inventory("i", "inventory"), customers("c", "customers"), total("t",
				"Total"), byDesc("D", "by_description"), // sorts parts by name
															// ascending
		byCount("C", "by_count"), byJoinDate("J", "by_join_date"), make("m", "make"), desc("d", "desc");
