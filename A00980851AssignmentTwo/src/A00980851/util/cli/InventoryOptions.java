 ...
 * InventoryOptions Date 2017-06-01
 */
public class InventoryOptions {
	/**
	 * Main driver of this class checks what arugments are present and acts on
	 * the apporiate action
	 * 
	 * @param args
	...
	public void inventoryCliCheck(String[] args, LinkedList<Inventory> inventoryLinkedList) throws ParseException {
		...
		if (SetupCLI.hasInventory()) {
			...
			if (SetupCLI.hasDesc()) {
				inventoryLinkedList.sort((s1, s2) -> s1.getDescription().compareTo(s2.getDescription())); // sorting
																											// by
																											// description
				description = true;
			}
			if (SetupCLI.hasCount()) {
				inventoryLinkedList
						.sort((s1, s2) -> Integer.valueOf(s1.getQuantity()) - Integer.valueOf(s2.getQuantity())); // sorting
																													// by
																													// quantity
				quantity = true;
			}
			...
			if (SetupCLI.isDescenindingAny()) {
				...
				if (quantity == true)
					inventoryLinkedList.sort((s1,
							s2) -> Integer.valueOf(s1.getQuantity()).compareTo(Integer.valueOf(s2.getQuantity())) * -1);
	...
	private void addTotal(LinkedList<Inventory> inventoryLinkedList) {
		for (int i = 0; i < inventoryLinkedList.size(); i++) {
			...
			inventory.setTotal(
					String.valueOf(Float.valueOf(inventory.getPrice()) * Integer.valueOf(inventory.getQuantity())));
		}
	}
}
